...
session_start();
include'../koneksi.php';
$carikode = mysql_query("select max(id_keluarga) from data") or die (mysql_error());
$datakode = mysql_fetch_array($carikode);
if ($datakode) {
   $nilaikode = substr($datakode[0], 1);
   $kode = (int) $nilaikode;
   $kode = $kode + 1;
   $hasilkode = str_pad($kode, 18, "474.1/2016/IX/", STR_PAD_LEFT);
   }else{
   $hasilkode = "0";
  }
?>
...
<body>
    <div class="container">
	<form method="post" action="cetaklahir.php" class="form-horizontal" role="form" novalidate>
		...
		<div class="form-group">
			...
			<label for="id_penduduk" class="col-sm-3 control-label">Kode Surat</label>
				<div class="col-sm-3">
				<input type="text" class="form-control" id="id_penduduk" name="kd" value="<?php echo $hasilkode;?>" required autofocus>
				</div>
			</div>
		...
		<div class="form-group">
			<label for="id_penduduk" class="col-sm-3 control-label">No-kk</label>
				<div class="col-sm-3">
				<input type="text" class="form-control" id="id_penduduk" name="kk" placeholder="isikan no kk" required autofocus>
				</div>
		</div>
